...
public class MongoDbInteractionApplicationTests {
	...
	@Test
	public void testSearchTeacherDetails() throws Exception {
		...
		Mockito.when(teacherService.getTeachers("/teachers/search?search=address:gt:10")).thenReturn(teacherList);
		
		
		mockMvc.perform(MockMvcRequestBuilders.get("/teachers/search?search=address:gt:10").content(json).contentType(MediaType.APPLICATION_JSON))
				      .andExpect(MockMvcResultMatchers.status().isOk());
		
		Mockito.verify(teacherService).getTeachers("/teachers/search?search=address:gt:10");
